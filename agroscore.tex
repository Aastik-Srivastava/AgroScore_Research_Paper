...
\hypersetup{
  ...
  pdfkeywords={agricultural credit, synthetic data governance, algorithm auditing, probability of default, calibration, benchmarking, reproducibility}
}
...
%=====================================================================
\begin{abstract}
Agricultural lending couples ordinary repayment risk with shocks to crop condition, weather and output prices, while borrower-level performance data are rarely shareable. This paper contributes AgroScore, a fully specified synthetic benchmarking framework for privacy-restricted agricultural credit research, algorithm auditing and synthetic-data governance. The generator produces temporally ordered cohorts over eight stylised Indian agricultural regions with 28 covariates, unobserved heterogeneity and a prespecified adverse regime. It is not fitted to agronomic, meteorological or loan-performance data; its environmental variables are structural proxies for testing model pipelines, not claims about observed climate systems. The central methodological claim is that circularity in mechanistic synthetic benchmarks must be owned, instrumented and varied: a model trained on labels generated from declared equations is being tested for alignment to those equations, not for real-world credit skill. We therefore prespecify four outcome regimes: additive-logit, threshold, conditional-interaction and hybrid. Across ten independently generated cohorts per regime, estimator ordering changes with the data-generating mechanism. Logistic regression aligns most closely with the additive-logit regime (mean $\auc=0.695$), random forest with the threshold regime ($0.782$), histogram boosting with the conditional-interaction regime ($0.782$), and extra trees narrowly with the hybrid regime ($0.678$). The seven-configuration analysis is retained as a generator sanity check rather than a comparative model-ranking claim. Within that reference regime, a ten-seed ablation finds that removing the financial block reduces discrimination consistently ($\Delta\auc=-0.027\pm0.009$), whereas removing the engineered climate--debt feature changes $\auc$ by $+0.0003\pm0.0012$. Out-of-time and counterfactual stress tests demonstrate how discrimination, calibration and decision utility can separate under declared shocks. All conclusions are properties of synthetic mechanisms and audit protocols, not evidence about real lending portfolios.
\end{abstract}

\begin{IEEEkeywords}
Agricultural credit, synthetic data governance, algorithm auditing, privacy-restricted data, probability of default, model calibration, benchmarking, reproducible research.
\end{IEEEkeywords}
...
Remote sensing is a frequently proposed supplementary signal. The normalised difference vegetation index (NDVI) is a long-established vegetation proxy \cite{tucker1979ndvi}, and MODIS MOD13Q1 supplies 16-day composites at 250\,m resolution \cite{mod13q1}. Empirical studies have tested such signals: M\"ollmann \textit{et al.} examine remotely sensed vegetation health indices in agricultural microfinance \cite{mollmann2020vegetation}, and Leng \textit{et al.} combine satellite-derived spatial variables with 49\,076 rural loans \cite{leng2026satellite}. AgroScore does not attempt to reproduce those products. It uses NDVI-like, soil-moisture-like and flood-risk-like variables as mathematical placeholders so that a credit-risk pipeline can be audited for timestamping, calibration, stress response and governance behaviour before any restricted borrower or satellite record is accessed.
...
Synthetic data are one response to this situation. However, a simulator can silently collapse to a low-dimensional manifold, reproduce marginal distributions while destroying dependence structure, reproduce source records when a learned generative model is used, or --- in the hand-specified case --- encode a latent outcome function so simple that any classifier saturates. AgroScore treats this circularity as the object of study. If the analyst writes the label equation, the benchmark cannot discover real credit-risk dynamics; it can only audit whether estimators, preprocessing, calibration checks and stress protocols behave as expected under a declared mechanism. A generator description alone does not reveal failures in that audit chain, so conclusions drawn from synthetic data should be accompanied by diagnostics appropriate to the generator class.

\subsection{Position and Contributions}
This paper does not claim that AgroScore data are realistic. That claim is unverifiable without a real portfolio to compare against. We make a narrower and checkable claim: the reported tests provide evidence that the generator implements selected properties of its declared specification, is stable across the evaluated seeds, does not exhibit excess cross-cohort near-duplicates, and possesses a dispersed covariance structure. These checks support, but do not certify exhaustively, specification conformance. The intended use is methodological: safe rehearsal of algorithm-audit procedures, synthetic-data release controls and model-risk governance in settings where real borrower records are unavailable, unshareable or legally constrained.

Our contributions are:

\begin{enumerate}
\item \textbf{A fully specified synthetic benchmarking framework} for privacy-restricted agricultural-credit research, algorithm auditing and governance rehearsal (Section~\ref{sec:architecture}), giving every distribution family, parameter, structural equation and regional stratum, with temporal ordering and a prespecified late-period adverse regime.
...
\item \textbf{A four-regime outcome-mechanism sensitivity analysis} (Section~\ref{sec:regimes}) repeated across ten seeds, showing that estimator ordering reverses when the declared outcome structure changes, plus a detailed generator sanity check of the additive-logit reference regime.

\item \textbf{Multi-seed ablation and learning-curve analysis} (Section~\ref{sec:ablation}) showing which proxy blocks are recoverable under the specified process, that the climate-proxy block effect is small and variable, and that the proposed climate--debt interaction has no measurable incremental benefit under the evaluated protocol.

\item \textbf{Temporal, geographic, decision-analytic and counterfactual-causal stress testing} (Section~\ref{sec:stress}) demonstrating how privacy-preserving synthetic cohorts can rehearse NIST-style validity, monitoring and risk-governance checks under prevalence shift and closed-form structural interventions on drought and irrigation proxy channels.
...
The generator and evaluation harness are organised around eight requirements. \textbf{R1:} every covariate must be realisable strictly before the outcome window, so that no evaluation can be contaminated by post-outcome information. \textbf{R2:} all compared models must use one identical split, locked before any model fitting, resampling, calibration or threshold analysis. \textbf{R3:} temporal and geographic transfer must be assessed separately from random interpolation, since random splitting measures interpolation within a regime rather than generalisation across regimes. \textbf{R4:} any oversampling, class balancing or data augmentation technique, if introduced in a downstream extension, must be applied only inside the training fold after the synthetic cohort has been generated and all test partitions have been sealed; no result in this paper uses such rebalancing. \textbf{R5:} discrimination, calibration and decision utility must be reported as three distinct properties. \textbf{R6:} every number in the paper must be regenerable from machine-readable artifacts by a fixed seed. \textbf{R7:} the generator itself must be subjected to targeted diagnostics, not merely described. \textbf{R8:} the benchmark must support outcome targets beyond the binary default flag, so that severity- and time-to-event-oriented quantitative risk tasks can be evaluated against the same point-in-time covariate vector $\mathbf{x}_i$ without re-specifying the generator.

R8 is realised by attaching two supplementary outcome targets to every simulated loan, conditional on the same latent draw used for $\eta_i$ in \eqref{eq:latent}. The first is a bounded loss-given-default severity, defined only for realised defaults ($y_i=1$):
\begin{equation}
...
The ordering is mechanism-dependent. Logistic regression is most aligned with the additive-logit generator; tree ensembles are more aligned with the threshold and conditional-interaction regimes, with histogram boosting exceeding logistic regression by 0.0266 $\auc$ on average in the latter. The hybrid regime is nearly tied between extra trees and logistic regression. Accordingly, AgroScore does not support a general claim that linear models dominate nonlinear models. It demonstrates that any synthetic benchmark reporting a single model ordering without prespecified outcome-regime sensitivity is reporting alignment to one chosen mechanism, not a property of credit modelling in general.

\subsection{Additive-Logit Generator Sanity Check}
For a detailed generator sanity check, seven learners receive identical inputs from the additive-logit reference regime: all 28 covariates, one stratified 80/20 split constructed once (seed 20260916) and reused throughout. Because this regime is additive on the logit scale, strong performance by regularised logistic regression is expected and should be read as evidence that the pipeline recovers the declared equation, not as evidence that logistic regression is generally preferable for agricultural credit.
...
Three protocol decisions deserve emphasis. First, \textbf{no class weighting, oversampling or synthetic augmentation is applied to any model in the reported experiments.} Such interventions can be legitimate in a downstream study, but only after the synthetic data have been generated and the test partitions locked, and then only within the training fold. Applying outcome-aware resampling before splitting would contaminate the held-out representation and would be a leakage failure. Second, class rebalancing deliberately distorts predicted probabilities toward the minority class; a model trained with balanced weights will show a Brier score and expected calibration error that reflect the weighting scheme rather than the estimator. Reporting calibration metrics for reweighted models alongside unweighted ones is not a valid comparison, and we avoid it. Third, hyperparameters are fixed \textit{a priori} and no test-set tuning is performed. This favours auditability over maximising any one score, and we note in Section~\ref{sec:threats} that it may understate each learner's attainable ceiling.
...
\subsection{Reference-Regime Recovery Results}
...
Table~\ref{tab:benchmark} reports the reference-regime recovery check. Regularised logistic regression attains the largest $\auc$ at 0.6710, as expected under an additive-logit generator. The stack is essentially tied at 0.6700. The remaining configurations recover the declared mechanism less directly under the fixed protocol, and the multilayer perceptron records 0.5728. These values are not a model-class ranking for real portfolios; they quantify estimator alignment to this particular synthetic equation and preprocessing protocol.

Calibration behaves differently from discrimination and therefore audits a different property of the pipeline. Extra trees, which is not the most discriminating estimator in this regime, is closest to ideal on this test set, with ECE 0.0019, intercept $+0.011$ and slope $1.011$. Histogram boosting and XGBoost show calibration slopes below 0.60, indicating over-dispersed predictions: they push probabilities toward the extremes more than the observed outcomes support. The MLP's slope of 0.339 indicates still stronger over-dispersion alongside weak ranking. This dissociation motivates reporting discrimination and calibration separately \cite{niculescu2005calibration}.
...
Figure~\ref{fig:roc} shows closely grouped curves apart from the MLP. Visual similarity does not exclude threshold-specific differences; auditing behaviour in a low-false-positive region would require prespecified partial-AUC or sensitivity comparisons with uncertainty.

\subsection{Uncertainty in Reference-Regime Alignment}
Summary metrics without uncertainty invite over-reading of small gaps. We compute paired bootstrap confidence intervals for $\Delta\auc$ against logistic regression, resampling test observations 2\,000 times and preserving observation-level pairing so that the comparison is within-sample. The purpose is not to crown a winning model, but to quantify how much separation exists between estimator outputs when the generator's reference equation is additive on the logit scale.
...
Table~\ref{tab:delta} and Figure~\ref{fig:delta} make the reference-regime alignment result precise for the evaluated fixed configurations. No point estimate exceeds logistic regression. The stack's interval $[-0.0053,0.0029]$ excludes a test-set AUC benefit larger than approximately 0.003 under this paired-bootstrap analysis. Three configurations have intervals lying entirely below zero.
...
The result aligns with a long-standing observation in credit scoring \cite{baesens2003benchmarking,lessmann2015benchmarking}: simple models can be competitive when the outcome is close to additive on the logit scale. Here, the evaluated logistic specification is explicitly aligned with the data-generating process. The weaker MLP result applies to the stated architecture, sample and fixed training protocol; it is not a general result about neural networks.

For the tested configurations on this process, XGBoost is 0.038 $\auc$ below logistic regression and the evaluated MLP is 0.098 below it. These are mechanism-alignment measurements, not recommendations about model classes on real portfolios.

\subsection{A Limited Configuration-Sensitivity Check: A Tuned Neural Network}
\label{sec:tuning-check}
The comparison above uses identical, fixed hyperparameters for every learner, which favours auditability but invites the objection that a learner's weak alignment reflects a bad configuration rather than a property of the data-generating mechanism --- particularly for the MLP, whose untuned $\auc$ of 0.5728 is barely above chance. We do not attempt a full nested-tuning study for every learner, but we run one concrete check: a grid search over hidden-layer architecture $\{(16),(32),(64),(64,32),(32,16)\}$ and $\ell_2$ penalty $\alpha\in\{10^{-4},10^{-3},10^{-2},10^{-1}\}$, selected by five-fold cross-validated $\auc$ on the training fold only, then evaluated once on the held-out random test set.

The selected configuration is a single hidden layer of 16 units with $\alpha=0.1$ --- far smaller and more regularised than the untuned $(64,32)$ architecture used above. Its test $\auc$ is 0.6090, closing roughly a third of the untuned 0.098 gap to logistic regression, but still 0.062 below it. Tuning helps considerably, which shows the untuned result did understate MLP alignment on this process, but it does not change the audit interpretation: under an additive-logit reference equation, nonlinear capacity is not automatically useful. We did not repeat this exercise for random forest, XGBoost or histogram boosting, whose gaps to logistic regression (0.027, 0.038, 0.026 respectively) are much smaller than the MLP's; whether tuning could close those smaller gaps is an open question this paper does not resolve.
...
Single-split ablation results are unstable, and a difference reported from one seed may not survive a second. We therefore repeat the entire pipeline --- regeneration, split locking, fitting, scoring --- across ten independent seeds and report the distribution of $\Delta\auc$ for each ablated block. The base learner here is logistic regression because it is the estimator most directly aligned with the additive-logit reference mechanism.
...
Table~\ref{tab:ablation} and Figure~\ref{fig:multiseed} give a clear ordering of recoverable signal under the declared equation. Removing the \textbf{financial block} costs 0.0265 $\auc$, negative in all ten seeds. The farm and market block contributes about a quarter as much with less stability. The \textbf{climate-proxy block contributes $-0.0017$}, an order of magnitude smaller than the financial block, and with a range spanning zero.
...
Calibration tells the opposite story. ECE degrades from 0.0121 to 0.0361; the calibration intercept moves from $-0.392$ to $+0.437$ and the slope from 0.785 to 1.069. The positive intercept means \textbf{systematic under-prediction} of default probability in the shifted regime --- the model assigns too little risk to a population that has become riskier under the declared shock. An institution relying on ranking alone would see discrimination apparently improve while the absolute probabilities underlying pricing, provisioning or monitoring became materially wrong. The broad pattern is preserved---the stack and logistic model remain near the top and the MLP remains last---but the ordering of intermediate tree and boosting configurations changes.
...
Table~\ref{tab:rolling} and Figure~\ref{fig:temporal} show $\auc$ varying between 0.663 and 0.706 with no monotone trend in training size, while ECE tracks prevalence: lowest in 2022 (prevalence 0.1141, ECE 0.0290) and highest in 2023 (prevalence 0.1664, ECE 0.0403). The spread across cuts, 0.043 $\auc$, exceeds every ablation effect in Table~\ref{tab:ablation} except the financial block. This is the main audit argument against single-split evaluation.
...
\label{sec:multitask}
Requirement R8 (Section~\ref{sec:architecture}) attaches two supplementary targets, LGD in \eqref{eq:lgd} and Weibull time-to-default in \eqref{eq:weibull}, to the same point-in-time covariate vector used for the primary binary benchmark. This section reports baseline empirical performance on both, established with off-the-shelf estimators rather than a full model comparison, to demonstrate that the extension is usable, not to identify a preferred learner.
...
\textbf{Estimator ordering depends on the outcome mechanism.} Logistic regression aligns most closely with the additive-logit reference regime, whereas tree ensembles align more closely with the threshold and conditional-interaction regimes. The original null result is therefore a specification-conditional sanity check, not evidence against nonlinear modelling. The near-tie in the hybrid regime also shows that adding nonlinear terms does not automatically create a material ensemble advantage.

\textbf{Recoverable signal is concentrated in the financial block under the specified generator.} The financial block's ablation effect is $15\times$ the climate-proxy block's and is the only one stable across all ten seeds. The replicated learning curve shows no emerging climate-proxy advantage through $n=32\,000$, but does not by itself rule out sample-size effects under other generator settings.
